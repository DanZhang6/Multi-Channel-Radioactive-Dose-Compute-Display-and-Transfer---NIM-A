...
#define GM2_DOSE_NEED_WEIGHTED_MOVING_AVERAGE 0 // GM2剂量值是否需要加权移动平均，0表示不需要，1表示需要
#define GM2_DOSE_NEED_WEIGHTED_MOVING_AVERAGE 0 // GM1剂量值是否需要加权移动平均，0表示不需要，1表示需要
...
sbit Judge_Speak = P5 ^ 5;  // 原1.7（231123）蜂鸣器标志信号，开关按键，高电平表示蜂鸣器不工作
sbit Zhu_Cong = P4 ^ 1;     // 原1.6（231123）主从机标志信号，开关按键，高电平为主机
sbit PCOLSIG = P3 ^ 3;      // 显示从机数据，应该没有实体按键，当虚拟按键使用，高电平表示不采集数据
sbit Note0 = P3 ^ 4;        // 并口从机发送数据标志
sbit Ack0 = P3 ^ 5;         // 并口主机接收完数据应答信号标志
sbit Note1 = P3 ^ 7;        // 并口从机接收数据标志
sbit Ack1 = P3 ^ 6;         // AH1*,原4.6（231123），并口主机接收完数据应答信号标志
