    ...
    */
    for (i = 0; i <= 7; i++)  // 8个探头逐个检测
    {
        ...
        if (Channel_Detector[i][0] == Channel_Detector[i][1])  // 当前一次量程和此次量程一样时
        {
            if ((Channel_Detector[i][1] == 4) && (Calculated[i] == 0))  // DL3量程，定时计数3秒，直接用来计算
            {
                ...
                if (i < 7)  // 前6个探头和第七个探头取计数位置不一样
                {
                    ...
                    Real_Count[i] = (double)(buf[4 * i + 5] * 256 + buf[4 * i + 4]);
            ...
            if ((Channel_Detector[i][1] == 3) && (Calculated[i] == 0))  // DL2量程，两次定时计数一秒相加
            {
                ...
                Channel_Display[i] = 3;
                Real_Count[i] = 0;
                if ((Display_Flag[i] == 1) && ((Real_Count_Display[i] / 5) > 5719) && (Channel_Detector[i][0] == Channel_Detector[i][1]) &&
            ...
            if ((Channel_Detector[i][1] == 1) && (Calculated[i] == 0))  // GM2量程，10次定时计数一秒相加，5次平滑平均
            {
                ...
                if (Average_Times[i] < 5) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        Real_Count_Display[i] = Average_Counts[i][0] * a3 + Average_Counts[i][1] * a3 + Average_Counts[i][2] * a4;
                        break;
                    case 4:
                        Real_Count_Display[i] = Average_Counts[i][0] * a2 + Average_Counts[i][1] * a2 + Average_Counts[i][2] * a3 + Average_Counts[i][3] * a4;
                        break;
                    }
                }
                if (Average_Times[i] == 5) {
                    Real_Count_Display[i] = Average_Counts[i][0] * a0 + Average_Counts[i][1] * a1 + Average_Counts[i][2] * a2 + Average_Counts[i][3] * a3 +
        ...
        } else if (Channel_Detector[i][0] != Channel_Detector[i][1])  // 当前后两次计数的量程不一样
        {
            ...
            {
                for (k = 0; k <= 2; k++)  // 初始化单次跳档判断变量，防止意外降档【2024】好像没什么必要，待确定
                {
            ...
            if ((Channel_Detector[i][1] == 0) && (Calculated[i] == 0))  // GM1量程，10次定时计数一秒相加，5次平滑平均
            {
                for (k = 0; k <= 2; k++) {
        ...
        for (j = 0; j < 8; j++)  //(2)存计数区为buf[3]-buf[32];探头个数标志
        {
            ...
            if (j < 7)  //(3)前七个探头既有电离室也有计数器，第八个只有电离室故分开处理
            {
                ...
                if (Channel_Display[j] == 0) {
                    ...
                    Para[1] = (float)(DataThouth[m + 2] * 1000 + DataCent[m + 2] * 100 + DataTenth[m + 2] * 10 + DataGe[m + 2]) / 1000000.0;
                    DoseRata[j] = (float)(Para[1] * ((float)Real_Count_Display[j] - Para[0]));
                } else if (Channel_Display[j] == 1) {
                    Para[2] = (float)(DataThouth[m + 3] * 1000 + DataCent[m + 3] * 100 + DataTenth[m + 3] * 10 + DataGe[m + 3]);  // 调节参数
                    Para[3] = (float)(DataThouth[m + 4] * 1000 + DataCent[m + 4] * 100 + DataTenth[m + 4] * 10 + DataGe[m + 4]) / 1000000.0;
                    ...
                    Para[5] = (float)(DataThouth[m + 6] * 1000 + DataCent[m + 6] * 100 + DataTenth[m + 6] * 10 + DataGe[m + 6]) / 1000.0;
                    DoseRata[j] = (float)(Para[5] * ((float)Real_Count_Display[j] - Para[4]));
                } else if (Channel_Display[j] == 3) {
                    Para[6] = (float)(DataThouth[m + 7] * 1000 + DataCent[m + 7] * 100 + DataTenth[m + 7] * 10 + DataGe[m + 7]);  // 调节参数
                    Para[7] = (float)(DataThouth[m + 8] * 1000 + DataCent[m + 8] * 100 + DataTenth[m + 8] * 10 + DataGe[m + 8]) / 10.0;
                    DoseRata[j] = (float)(Para[7] * ((float)Real_Count_Display[j] - Para[6]));
                } else if (Channel_Display[j] == 4) {
                    Para[8] = (float)(DataThouth[m + 9] * 1000 + DataCent[m + 9] * 100 + DataTenth[m + 9] * 10 + DataGe[m + 9]);  // 调节参数
                    Para[9] = (float)((DataThouth[m + 10] * 1000 + DataCent[m + 10] * 100 + DataTenth[m + 10] * 10 + DataGe[m + 10]) * 10.0);
                    DoseRata[j] = (float)(Para[9] * ((float)Real_Count_Display[j] - Para[8]));
                }
            } else if (j == 7) {
                Display_Flag[j] = 0;
                if (Channel_Display[j] == 3) {
                    Para[0] = (float)(DataThouth[m + 1] * 1000 + DataCent[m + 1] * 100 + DataTenth[m + 1] * 10 + DataGe[m + 1]);  // 调节参数
                    Para[1] = (float)(DataThouth[m + 2] * 1000 + DataCent[m + 2] * 100 + DataTenth[m + 2] * 10 + DataGe[m + 2]) / 10.0;
                    DoseRata[j] = (float)(Para[1] * ((float)Real_Count_Display[j] - Para[0]));
                } else if (Channel_Display[j] == 4) {
                    Para[2] = (float)(DataThouth[m + 3] * 1000 + DataCent[m + 3] * 100 + DataTenth[m + 3] * 10 + DataGe[m + 3]);  // 调节参数
                    Para[3] = (float)(DataThouth[m + 4] * 1000 + DataCent[m + 4] * 100 + DataTenth[m + 4] * 10 + DataGe[m + 4]) * 10.0;
                    DoseRata[j] = (float)(Para[3] * ((float)Real_Count_Display[j] - Para[2]));
                }
            }
            /**********单位换算后为jtemp************/
