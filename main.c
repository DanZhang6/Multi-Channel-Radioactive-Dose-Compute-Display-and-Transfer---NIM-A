...
*--------------------------------------------------------------------------------------------------------------------
*相对于原始程序的修改记录:
*2016年4月14日，修改剂量显示单位为uGy/h,通道名称:Ch1...
*2016年4月18日(AA1)，
*2016年4月19日(AB1),
*2016年4月20日(AC1),
*2016年4月22日(AD1),
...
double Average_Counts[8][5];//平滑平均变量
...
//	   BackUp_Display();
       ...
       {
           key_function();
       } 								//按键功能，在while循环中不断检测按键标志，按键标志由按键外部中断来更改
...
void Int0() interrupt 0
 {
	...
	Ptem1=Ptem1&0x7F;
	for(i=0;i<18433;i++);                            //延时去抖【2024】22.1184晶振机器周期0.5425us，这里消抖延时太短，一般10ms，改为18433
	Ptem2=Keyvar;
	Ptem2=Ptem2&0x7F;
	if(Ptem1!=Ptem2)
	{
...
void Init_RAM()
{
	...
	for(i=0;i<8;i++)								//AA1+
	{
		Real_Count_Display[i]=0;                     //AA1*默认刷新时间为60秒
		Setted_Time[i]=60;							//AB1+初始化设置时间变量
		Average_Times[i]=0;								//AJ1+滑动平均次数
		Display_Flag[i]=0;
		Channel_Display[i]=0;
	}
    ...
    Var_Signal3 = 0xff;								//探头控制信号标志，接电离室II，开机默认
